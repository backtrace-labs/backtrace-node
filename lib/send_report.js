...
function parseStack(info, cb) {
  ...
  for (var i = 0; i < lines.length; i += 1) {
    ...
    if (sourceCodePath === filterFilePath) continue;
  ...
  resolveSourceCode(report, tabWidth, contextLineCount, wantedSourceCode, info, onResolvedSourceCode);
...
function resolveSourceCode(report, tabWidth, contextLineCount, wantedSourceCode, info, cb) {
  report.sourceCode = {};
  ...
  function makeReadFn(sourceCodePath, minLine, maxLine) {
    ...
    function readFn(cb) {
      ...
      function onReadFinished(err, buf) {
        if (err) {
          errorIfDebug(info, "Unable to read '" + sourceCodePath + "': " + err.message);
...
function obtainMemInfo(info, cb) {
  ...
  function onReadFinished(err, contents) {
    ...
    for (var i = 0; i < lines.length; i += 1) {
      ...
      if (!match) {
        errorIfDebug(info, "unrecognized line in /proc/meminfo: '" + line + "'");
        continue;
      }
      var name = match[1];
      ...
      if (units === "B" || units === "bytes") {
        ...
        errorIfDebug(info, "unrecognized units in /proc/meminfo: '" + units + "'");
        continue;
      }
...
function makeCollectUnameFn(info, unameArg, attrName, cb) {
  return function(cb) {
    collectStdout("uname", [unameArg], function(err, stdout) {
      if (err) {
        errorIfDebug(info, "Unable to exec uname " + unameArg + ": " + err.message);
