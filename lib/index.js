...
function obtainModuleInfo(report) {
  ...
  if (count > 0) report.annotations.Dependencies = dependencies;
}
...
function fetchModulePackageJson(mod) {
  ...
  if (!mod || !mod.filename) return null;
  ...
  while (true) {
    try {
      ...
      if (err.code === "MODULE_NOT_FOUND") {
        ...
        errorIfDebug(
          "Unable to find package.json for module " +
            mod.filename +
            ": " +
            err.message
        );
...
function finishReport(self) {
  if (
    self.logLines.length !== 0 &&
    self.payload.report.annotations.Log == null
  ) {
...
function collectErrorAnnotations(self, err) {
  ...
  if (!objHasAnyProps(errorProps)) return;
